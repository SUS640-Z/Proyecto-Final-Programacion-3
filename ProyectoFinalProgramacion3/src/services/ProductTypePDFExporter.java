...
public class ProductTypePDFExporter {
	public void exportProductType(List<ProductType> productType, File file) throws IOException {
		...
		try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(file));
			...
			doc.add(new Paragraph("Reporte de Tipos de Productos").setBold().setFontSize(12)
					.setTextAlignment(TextAlignment.CENTER));

			doc.add(new Paragraph("").setMarginTop(30));

			float[] columnsWidth = {1,6};

			Table table = new Table(UnitValue.createPercentArray(columnsWidth)).useAllAvailableWidth();

			PdfFont font = PdfFontFactory.createFont(StandardFonts.HELVETICA);

			Cell cell = new Cell(1, 5).add(new Paragraph("Tipos de productos del sistema")).setFont(font).setFontSize(14)
